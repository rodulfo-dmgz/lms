...
function showApp(profile) {
    ...
    initPushNotifications(profile.id);       // push web — demande permission si besoin
...
async function init() {
    ...
    if (session?.user) {
        ...
        try {
            ...
            // Sécurité : si l'utilisateur n'est pas admin, effacer tout viewAs résiduel
            if (profile?.role !== 'admin') { store.setViewAs(null); store.setViewAsProfile(null); }
            showApp(profile);
